...
class WriteTables<DestinationT>
  ...
  private void load(
    ...
    boolean fail = true;
    if(fail) {
      String fileToDelete = gcsUris.get(0);
      LOG.info(
              "xyz123 Intentionally deleting file {} total number of files is {}.",
              fileToDelete,
              gcsUris.size());
      MatchResult matchResult = FileSystems.match(fileToDelete);
      assert matchResult.metadata().size() == 1;
      ResourceId resourceIDtoDelete = matchResult.metadata().get(0).resourceId();


      FileSystems.delete(ImmutableList.<ResourceId>of(resourceIDtoDelete));
      matchResult = FileSystems.match(fileToDelete);
      assert matchResult.metadata().size() == 0;
      LOG.info(
              "xyz123 Intentionally deleted file {}.",
              fileToDelete);
    }
