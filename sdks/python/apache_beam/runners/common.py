...
class SimpleInvoker(DoFnInvoker):
  ...
  def invoke_process(
    ...
    if restriction_tracker:
      ...
      kwargs = {restriction_tracker_param: restriction_tracker}
      if watermark_reporter:
        ...
        if watermark_reporter_param:
          kwargs[watermark_reporter_param] = watermark_reporter
        else:
      ...
      value = windowed_value.value[0] if isinstance(windowed_value.value, tuple) else windowed_value.value
      self.output_processor.process_outputs(
          windowed_value, self.process_method(
              value, **kwargs))
    else:
      self.output_processor.process_outputs(
...
class DoFnRunner(Receiver):
  ...
  def process(self, windowed_value):
    try:
      ...
      if isinstance(windowed_value.value, tuple): # TODO Add a type instead of using tuple here.
        restriction_tracker = windowed_value.value[1]
...
class PTransformOverride(object):
  ...
  def map_outputs(self, outputs, new_output):
    pass