...
class ReadFromAvro(PTransform):
  ...
  def __init__(self, label=None, file_pattern=None, min_bundle_size=0):
    ...
    Uses source '_AvroSources' to read a set of avro files defined by a given
    file pattern.
    ...
    Python SDK types. Records that are of Avro type 'RECORD' will be mapped to
    Python dictionaries that complies with the schema contained in the Avro file
    that contains that particular record. In this case, keys of each dictionary
    ...
    """
    super(ReadFromAvro, self).__init__(label)
